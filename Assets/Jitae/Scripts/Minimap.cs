...
public class Minimap : MonoBehaviour
{
    ...
    */
    ...
    private void CreateIcons(List<GameObject> targets, GameObject iconPrefab)
    {
        if (iconPrefab == null)
        {
    ...
    private void ScaleIconToCollider(GameObject icon, GameObject target, Collider2D targetCollider)
    {
        ...
        */
        ...
        SpriteRenderer iconRenderer = icon.GetComponentInChildren<SpriteRenderer>();
        if (iconRenderer == null || iconRenderer.sprite == null)
        {
            return;
        }

        Vector3 spriteSize = iconRenderer.sprite.bounds.size;
        ...
        // scaleMultiplier는 최종 아이콘 크기 배율 (기본적으로 1배율)
                                                worldSize.x / SafeScale(parentScale.x * spriteSize.x) * scaleMultiplier,
                                                worldSize.y / SafeScale(parentScale.y * spriteSize.y) * scaleMultiplier,
                                                iconScale.z
                                                );
    }
